...
model_name = "isotropic_reconstruction"
...
model = HookedModel(
    model = cir.ConvAutoencoder(),
    layer_names = []
)

if os.path.isfile(f"autoencoders/{model_name}/autoencoder_model_100.pth"):
    checkpoint = torch.load(f"autoencoders/{model_name}/autoencoder_model_100.pth")
    ...
    print(f"autoencoders/{model_name}/autoencoder_model_100.pth loaded")
...
def plot_for_multiple_paths(layers, all_candidate_embeddings_train, all_candidate_embeddings_test):
    scores = []
    layer_num = []
    list_n_components = [256, 128, 64, 32, 16]
    layer_index = 0
    for n_components in tqdm(list_n_components):
    # took the first layer
        gen = np.vstack([t.cpu().numpy() for t in layers[layer_index]]).astype('float')
    ...
    plt.legend()
    plt.savefig(f'{save_path}/{generated_data_source}/ct_v_pca_layer_{model_name}:L{layer_index}.png')
    return scores
