...
for model_ckpt in tqdm(ckpt_list):
    ...
    def process_generated_images(path, batch_size=500):
        query_embeddings = []
        for filename in os.listdir(path):
            ...
            if len(query_embeddings) == batch_size:
                layers = [[] for _ in range(len(query_embeddings[0]))]
                ...
                query_embeddings = []  # Reset the list for the next batch
                yield scores
        if query_embeddings:
            layers = [[] for _ in range(len(query_embeddings[0]))]
            for result in query_embeddings:
    ...
    train_path = os.path.join(target_paths, 'train')
    test_path = os.path.join(target_paths, 'test')

    time_start = time.time()

    print(f"Processing train data from {train_path}")
    for dir in os.listdir(train_path):
        print(f"Processing {dir}")
        path = os.path.join(train_path, dir)
        for batch_scores in process_generated_images(path):
            with open(f'/home/jackhe/LayerChoice/fingerprinting_baseline/{generated_data_source}_scores.txt', 'a') as f:
                f.write(f"{dir}_train_batch: {batch_scores}\n")
    ...
    for dir in os.listdir(test_path):
        ...
        for batch_scores in process_generated_images(path):
            with open(f'/home/jackhe/LayerChoice/fingerprinting_baseline/{generated_data_source}_scores.txt', 'a') as f:
                f.write(f"{dir}_test_batch: {batch_scores}\n")
